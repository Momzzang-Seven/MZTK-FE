...
- `src/store/adminStore.ts` — 현재 mock 데이터 기반 (`generateMockUsers/Posts/Inquiries`), 실제 API 연동 미완
- `src/pages/trainer/CreateTicket.tsx:48`, `EditTicket.tsx:43` — TODO 2건, 서버 API 연동 대기 중
- `src/pages/market/MarketPurchase.tsx:206` — TODO 1건, 서버 연동 로직 대기 중
- CI/CD 미설정 — GitHub Actions 워크플로우 부재
- `@interface`, `@context` alias — 실제 디렉토리 부재 (orphan alias)
