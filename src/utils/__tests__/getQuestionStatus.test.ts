...
describe("getQuestionStatus utils", () => {
  describe("getStatus", () => {
    ...
    it('publicationStatus가 VISIBLE이고 isSolved가 false이고 commentCount가 1 이상이면 "answering"을 반환한다', () => {
      expect(getQuestionStatus("VISIBLE", "NORMAL", false, 1)).toBe(
        "answering"
      );
      expect(getQuestionStatus("VISIBLE", "NORMAL", false, 5)).toBe(
        "answering"
      );
      expect(getQuestionStatus("VISIBLE", "NORMAL", false, 100)).toBe(
        "answering"
      );
    });
