...
const TicketForm = ({
    ...
    const texts = mode === "create" ? CREATE_TICKET_TEXT : { ...CREATE_TICKET_TEXT, ...EDIT_TICKET_TEXT };
    ...
    const handleSubmit = async () => {
        ...
        const hasNoTimeRegistered = formData.operatingDays.some((day: string) => formData.operatingTimes[day].length === 0);
                        ...
                        {imagePreviews.length > 0 ? (
                            <>
                                ...
                                <div className="absolute inset-0 bg-black/20 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                                    <span className="text-[10px] text-white font-bold font-main">변경</span>
                                </div>
                            </>
                        ) : (
                            <div className="w-full h-full flex flex-col items-center justify-center opacity-40">
                                <img src="/icon/camera.svg" alt="camera" className="w-6 h-6 mb-1" />
                ...
                <div className="flex flex-col">
                    ...
                    <div className="px-5 py-6 flex flex-col gap-6">
                        <div className="flex flex-col gap-2">
                            <label className="text-[13px] font-bold text-gray-500 uppercase tracking-wider">{CREATE_TICKET_TEXT.LABELS.TITLE}</label>
                            <input
                                name="title"
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            <div className="flex flex-col gap-2">
                                <label className="text-[13px] font-bold text-gray-500 uppercase tracking-wider">{CREATE_TICKET_TEXT.LABELS.CATEGORY}</label>
                                <select
                                    name="category"
                            ...
                            <div className="flex flex-col gap-2">
                                <label className="text-[13px] font-bold text-gray-500 uppercase tracking-wider">{CREATE_TICKET_TEXT.LABELS.PRICE}</label>
                                <div className="relative">
                                    ...
                                    />
                                    <span className="absolute right-4 top-1/2 -translate-y-1/2 text-main font-bold text-xs">MZTK</span>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            <div className="flex flex-col gap-2">
                                <label className="text-[13px] font-bold text-gray-500 uppercase tracking-wider">{CREATE_TICKET_TEXT.LABELS.CAPACITY}</label>
                                <input
                                    name="capacity"
                                    type="number"
                            ...
                            <div className="flex flex-col gap-2">
                                <label className="text-[13px] font-bold text-gray-500 uppercase tracking-wider">{CREATE_TICKET_TEXT.LABELS.DURATION}</label>
                                <input
                                    name="duration"
                        ...
                        <div className="flex flex-col gap-2">
                            <label className="text-[13px] font-bold text-gray-500 uppercase tracking-wider">{CREATE_TICKET_TEXT.LABELS.SUPPLIES}</label>
                            <input
                                name="supplies"
                    ...
                    <div className="bg-gray-50 px-5 py-6 flex flex-col gap-4">
                        <label className="text-[13px] font-bold text-gray-500">해시태그 (최대 3개)</label>
                        <div className="flex gap-2">
                            {formData.tags.map((tag: string, index: number) => (
                                <div key={index} className="flex-1 relative flex items-center bg-white rounded-xl px-3 py-3 shadow-sm border border-gray-100 focus-within:border-main/30 transition-all">
                    ...
                    <div className="px-5 py-8 flex flex-col gap-5">
                        <label className="text-[13px] font-bold text-gray-500 uppercase tracking-wider">{CREATE_TICKET_TEXT.LABELS.FEATURES}</label>
                        <div className="flex flex-col gap-3">
                            {formData.features.map((feature: string, index: number) => (
                                <div key={index} className="flex gap-3 items-center group">
                                    <span className="text-main font-black text-lg w-6 opacity-40 group-focus-within:opacity-100 transition-opacity">{index + 1}</span>
                                    <input
                                        value={feature}
                    ...
                    <div className="bg-gray-50 px-5 py-8 flex flex-col gap-8 pb-32">
                        <div className="flex flex-col gap-4">
                            <div className="flex flex-col gap-1">
                                <label className="text-[13px] font-bold text-gray-500 uppercase tracking-wider">{CREATE_TICKET_TEXT.LABELS.OPERATING_DAYS}</label>
                                <p className="text-[11px] text-gray-400">{CREATE_TICKET_TEXT.LABELS.OPERATING_DAYS_DESC}</p>
                            </div>
                        ...
                        {formData.operatingDays.length > 0 && (
                            <div className="flex flex-col gap-5 animate-in fade-in slide-in-from-top duration-300">
                                <div className="flex flex-col gap-1 border-t border-gray-200 pt-6">
                                    <label className="text-[13px] font-bold text-gray-500 uppercase tracking-wider">{CREATE_TICKET_TEXT.LABELS.OPERATING_HOURS}</label>
                                    <p className="text-[11px] text-gray-400">{CREATE_TICKET_TEXT.LABELS.OPERATING_HOURS_DESC}</p>
                                </div>

                                {['월', '화', '수', '목', '금', '토', '일'].filter(day => formData.operatingDays.includes(day)).map((day) => (
                                    <div key={day} className="flex flex-col gap-3 p-4 bg-white rounded-2xl shadow-sm border border-gray-100">
                                        <div className="flex items-center gap-4">
                                            <div className="text-base font-black text-gray-800 w-6">{day}</div>
                                        ...
                                        {formData.operatingTimes[day].length > 0 && (
                                            <div className="flex flex-wrap gap-2 pl-10">
                                                {formData.operatingTimes[day].map((time: string) => (
                                                    <div key={time} className="flex items-center gap-1.5 bg-gray-50 text-gray-700 px-3 py-1.5 rounded-lg text-[12px] font-bold border border-gray-100 group">
                                                        <span>
