...
describe("WalletSuccessSection", () => {
  ...
  it("커스텀 title prop이 표시된다", () => {
    render(
      <WalletSuccessSection title="지갑 생성 완료!" onConfirm={mockOnConfirm} />
    );
  ...
  it("description prop이 표시된다", () => {
    ...
    expect(
      screen.getByText("이제 토큰을 받을 수 있습니다.")
    ).toBeInTheDocument();
  });
  ...
  it("기본 버튼 라벨이 표시된다", () => {
    ...
    expect(
      screen.getByRole("button", { name: "모두 이해했어요" })
    ).toBeInTheDocument();
  });

  it("커스텀 buttonLabel prop이 표시된다", () => {
    render(
      <WalletSuccessSection buttonLabel="다음으로" onConfirm={mockOnConfirm} />
    );
