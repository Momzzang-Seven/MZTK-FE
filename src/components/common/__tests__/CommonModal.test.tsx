...
describe("CommonModal", () => {
  it("제목과 설명이 올바르게 렌더링된다", () => {
    render(
      <CommonModal title="테스트 제목" desc="테스트 설명" confirmLabel="확인" />
    );
  ...
  it("cancelLabel이 없으면 취소 버튼이 렌더링되지 않는다", () => {
    render(<CommonModal title="확인만 있는 모달" confirmLabel="확인" />);
  ...
  it("confirmLabel이 없으면 확인 버튼이 렌더링되지 않는다", () => {
    render(<CommonModal title="취소만 있는 모달" cancelLabel="취소" />);
  ...
  it("모달 컨텐츠 영역이 올바른 스타일을 가진다", () => {
    ...
    const modalContent = document.body.querySelector(".rounded-\\[40px\\]");
