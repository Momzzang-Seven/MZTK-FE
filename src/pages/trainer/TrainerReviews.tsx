...
const MOCK_REVIEWS = [
  {
    ...
    content:
      "처음 웨이트를 시작해서 걱정이 많았는데 꼼꼼하게 자세를 봐주셔서 자극이 어디에 와야하는지 정확히 알게 되었습니다! 트레이너님 덕분에 헬스장에 가는 게 즐거워졌어요😊",
  },
  {
    id: "rv2",
  ...
  },
  {
    ...
    content:
      "말씀하신 대로만 식단하고 운동했더니 인생 첫 바프 대성공했습니다!!!! 🔥🔥 진짜 멘탈 관리까지 완벽하게 해주셔서 견딜 수 있었어요! 최고!!",
  },
  {
    id: "rv4",
    customerName: "초보헬린이",
    ...
    className: "1:1 집중 웨이트 트레이닝",
    content:
      "자세는 잘 잡아주시는데, 인기 많은 강사님이시라 예약 잡기가 조금 빡셉니다 ㅠㅠ 그래도 수업 퀄리티 자체는 대만족이에요!",
  },
];
...
const TrainerReviews = () => {
  ...
  const classNames = [
    "전체",
    ...Array.from(new Set(MOCK_REVIEWS.map((r) => r.className))),
  ];
    ...
    <div className="flex flex-col h-full bg-gray-50 min-h-screen">
      ...
      <div className="flex-1 overflow-y-auto px-5 py-6 pb-24 flex flex-col gap-6">
        ...
        <div className="flex flex-col gap-3 mb-2">
          ...
          <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 flex flex-col items-center justify-center gap-3">
            ...
            {filteredReviews.length === 0 ? (
              <div className="text-gray-400 text-[13px] font-medium py-2">
                아직 작성된 후기가 없습니다.
              </div>
            ) : (
              <div className="flex items-end gap-1.5">
                <span className="text-yellow-400 text-[28px] -mt-1 drop-shadow-sm">
                  ⭐
                </span>
        ...
        <div className="flex flex-col gap-4">
          ...
          {filteredReviews.length === 0 ? (
            <div className="text-center text-gray-400 py-10 font-medium text-[14px]">
              해당하는 후기가 없습니다.
            </div>
          ) : (
            filteredReviews.map((review) => (
              <div
                key={review.id}
                className="bg-white rounded-2xl p-5 border border-gray-100 shadow-sm flex flex-col gap-3.5"
              >
                {/* 후기 상단 헤더: 별점 및 날짜 */}
                <div className="flex justify-between items-center">
                  ...
                  <span className="text-gray-400 text-[12px] font-medium tabular-nums">
                    {review.date}
                  </span>
                </div>
